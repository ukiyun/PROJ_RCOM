...
void help()
{
    printf("\n\n"
           ...
           "IMPORTANT: Changing de baud rate or propagation delay while a transmission is\n"
...
int main(int argc, char *argv[])
{
    ...
    while (STOP == FALSE)
    {
        ...
        if (par.logfile != NULL)  // Currently logging
        {
            ...
            {
                sprintf(tx2rxTx, "%02X", par.tx2rx[par.tx2rxIdx]);
            }
            else
            {
                memcpy(tx2rxTx, "  ", 3);
            ...
            {
                sprintf(rx2txTx, "%02X", par.rx2tx[par.rx2txIdx]);
            }
            else
            {
                memcpy(rx2txTx, "  ", 3);
        ...
        if (par.logfile != NULL)  // Currently logging
        {
            if (par.tx2rxValid[par.tx2rxIdx])
            {
                sprintf(tx2rxRx, "%02X", par.tx2rx[par.tx2rxIdx]);
            }
            else
            {
                memcpy(tx2rxRx, "  ", 3);
            ...
            if (par.rx2txValid[par.rx2txIdx])
            {
                sprintf(rx2txRx, "%02X", par.rx2tx[par.rx2txIdx]);
            }
            else
            {
                memcpy(rx2txRx, "  ", 3);
